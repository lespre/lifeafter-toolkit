...
> ⚠️ 屎山警告：长期个人迭代的产物，目录里同时有“权威链路”和“历史实验”，以 `readme.txt` 与 `统一索引与拆包流程.md` 为准。

## 界面预览

| 拆包任务（防呆选包） | 皮肤 3D 预览器 |
| :---: | :---: |
| ![拆包器界面](docs/screenshots/unpacker-gui.png) | ![皮肤预览器](docs/screenshots/skin-viewer.png) |

## 下载

- **`明日之后拆包器_新.exe`（Windows，双击即用）** → 见 [Releases](../../releases/latest)

> ⚠️ **屎山代码预警**：exe 是长期快速迭代的产物 —— 代码结构混乱、命名随意（真·屎山），仅供学习折腾；认真使用请以仓库源码与文档为准。
